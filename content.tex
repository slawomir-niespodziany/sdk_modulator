...
\noindent
Feedback signal can calculated at samples $4n+1$ and $4n+3$ is
...
% TODO CITE? DEMODULATION ALGORITHM?
...
\noindent
The feedback signal, used for loop closure, is dependent on four recent drive values. Because the DAC can be controlled with $\Delta{}v$ resulotion, the scale factor error, and thus the occuring errors $\delta_n$ are all controllable with a fixed delta $\Delta\phi^{v}$ corresponding to $\Delta{}v$.
...
% Modulation algorithm serves as subsystem for the contol loop. 
Feedback sample is obtained for every bias point pair (at every sample $2n + 1$). The control loop can thus operate at that same rate. If this is the case, it may be tempting to simplify the implementation and only drive two consecutive samples to generate next feedback value. This solution is however incomplete, as feedback signal depends on the four last samples. If not accounted for, the previous samples act as digital noise source, disrupting the measured error and affecting control loop stability. A corrected solution is to consider the recent drive values, while calculating the next two. The total shift applied across the four samples has an effect in feedback signal.

\subsubsection{Drive symmetry}
The \textbf{intended} drive values between samples $n$ and $n+2$ are symmetrical, i.e. they only differ in sign, but have identical amplitude. However, for an actual signal driven with a DAC, this assumption falls short. This is due to the discrete nature of a DAC. The intended symmetric values have ideally controlled amplitude. When in a real--world scenario the amplitude is discreticized, the most bneficial solution is to utilize as much of the resolution, while keeping the drive signals relatively symmetrical as a second priority. This allows for controlling the aggregated phase shift $\Delta\phi_{n}$ with $1 [lsb]$. Otherwise, the effective resolution is lowered by one bit - the difference betweeen two effective phase shifts would correspond to $(k_{n} \cdot \Delta\phi_{v}) - (-k_{n} \cdot \Delta\phi_{v}) \rightarrow 2\Delta\phi_{v}$ for adjacent $k_{n}$.
% , while the DAC offers control with $\Delta\phi_{v}$ resolution.

\subsubsection{Rounding factor}
Another corner cut is the rounding method selection for driving discrete DAC values, when performing calculations with fractional resolution. Especially prone are the designs using floating point arithmetics (e.g. IEEE-754 float/double, typically used in C/C++ of programmable logic). The problem lays in the rounding direction when working with positive and negative values. Mathematically--wise such operation shall be symmetrical. However, as the previous section indicates already, drive symmetry leads to loss of effective resolution. This is regardless of which of the following methods is selected:
\begin{itemize}
...
\noindent
% If the algorithm rounds fractional values symmetrically, it will lead to a simillar effect as in the previous section. While the effect is the same, the reason may be different, because the symmetry may also result from other factors. 
Rounding problem is worth of emphasizing, as it is expected for multiple implementations to use floating point arithmetics.

It falls into a analysis part which focuses on nummerical aspects of algorithm implementation. While this is often covered by using floating point arithmetics with double precision, it may not always be the right solution.

This is an important nummerical issue, wspecially when performing calculations in floating point and only mapping the result to the integer, or fixed--point type required by a DAC. The below snippet shows an example.
