...
For an all--digital modulator a reference voltage has to be set up, to account for the maximum possible scale factor. From Equation~\ref{eq:bias_values} it results, that a span of $1.5\pi$ is required for biasing and additionally, the Serrodyne modulator requires a span of $2\pi$. This yields the required modulator span to be $S = 3.5\pi$. Minimum DAC reference $V_{ref}^{min}$ shall be adjusted to cover all the required span and account for some additional margin $M \in [1.0; 1.1]$.
...
\noindent
The $V\pi^{max}$ is the highest expected half--wave voltage. When determining the final reference it is advantageous to round it up to a voltage multiple of one of typically available chip references (Equation~\ref{eq:reference_voltage_ceil}), e.g. $N \cdot 5V$ or $N \cdot 4.096V$. It makes implementing the elctronic circuit more straightfroward. In practice, a compromise is to utilize the resulting excess for the margin $M$, so that after adding it, the $V_{ref} \approx V_{ref}^{min}$.
...
DAC usage results in quantization error. Resolution of the converter and its reference voltage determine the minimal controllable voltage change:
...
\noindent
From this an error relative to the actual half--wave voltage results from the distance between the controlled value and the actual one, being half of the $\Delta{}v$. It can be vary between bias points $n$, as the actual $V_{\pi}(n)$ may continously change over time:

\begin{equation}
    \delta_n = \frac{1}{2} \cdot \frac{\Delta{}v}{V_\pi(n)}
    \label{eq:relative_scale_factor_error}
\end{equation}

\noindent
The total error accumulated from various parameters will be:

\begin{equation}
    \delta(n) = \frac{1}{2} \cdot \frac{\lceil V_{\pi}^{max} \cdot S \cdot M \rceil \cdot 2^{-N}}{V_{\pi}(n)}
    \label{eq:total_error}
\end{equation}

\noindent
which in practice may be as in the following exemplary calculation:

\begin{equation}
    \begin{split}
        \delta_{ex} & = \frac{1}{2} \cdot \frac{\lceil \overbrace{4.5V}^{V_{\pi}^{max}} \cdot \overbrace{3.5}^{S} \cdot \overbrace{1.025}^{M} \rceil \cdot \overbrace{2^{-16}}^{2^{-N}} }{\underbrace{3.5V}_{V_{\pi}}} = \frac{\lceil 16.14V \rceil \cdot 2^{-16}}{ 7V} = \frac{\overbrace{16.384V}^{4 \cdot 4.096V} \cdot 2^{-16}}{7V} = \\
...
\noindent
The values of $\Delta\phi_s(n)$ and $\Delta\phi_r(n)$ denote the Sagnac--induced phase shift and the cancelling ramp shift, which are assumed to be zero for the following analysis. By using equations \ref{eq:response} and \ref{eq:bias_values} and assuming the $cos$ function linear near the bias points (Equation~\ref{eq:cosine_linearization}), we get:

\begin{equation}
    \begin{split}
        P_{n}^{+\frac{3}{2}\pi}     & = \frac{P_0}{2} \cdot (1 + cos(\Delta\phi_{n}^{+\frac{3}{2}\pi})) = \frac{P_0}{2} \cdot (1 + cos(\phi_{n}^{+\frac{3}{2}\pi} - \phi_{n - 2}^{-\frac{3}{2}\pi})) = \\
                                    & = \frac{P_0}{2} \cdot (1 + cos(\frac{3}{2}\pi + \frac{3}{2}\pi \cdot \frac{\delta_{n} + \delta_{n - 2}}{2})) =                                                   \\
                                    & = \frac{P_0}{2} \cdot (1 + \frac{3}{2}\pi \cdot \frac{\delta_{n} + \delta_{n - 2}}{2})                                                                           \\
        P_{n + 1}^{-\frac{1}{2}\pi} & = \frac{P_0}{2} \cdot (1 - \frac{1}{2}\pi \cdot \frac{\delta_{n + 1} + \delta_{n - 1}}{2})                                                                       \\
        P_{n + 2}^{-\frac{3}{2}\pi} & = \frac{P_0}{2} \cdot (1 - \frac{3}{2}\pi \cdot \frac{\delta_{n + 2} + \delta_{n}}{2})                                                                           \\
        P_{n + 3}^{+\frac{1}{2}\pi} & = \frac{P_0}{2} \cdot (1 + \frac{1}{2}\pi \cdot \frac{\delta_{n + 3} + \delta_{n + 1}}{2})
    \end{split}
    \label{eq:power}
\end{equation}
...
\noindent
From there, the intensity difference for the samples $n + 1$ and $n + 3$ is:

\begin{equation}
    \begin{split}
        e_{n+1}^{-\frac{1}{2}\pi} & = P_{n}^{+\frac{3}{2}\pi} - P_{n + 1}^{-\frac{1}{2}\pi} =                                                                                                                      \\
                                  & = \frac{P_0}{2} \cdot (\frac{\delta_{n} + \delta_{n - 2}}{2} - \frac{\delta_{n + 1} + \delta_{n - 1}}{2}) =                                                                    \\
                                  & = \frac{P_0 \cdot \pi}{4} \cdot (\underbrace{\delta_{n - 2} - \delta_{n - 1}}_{\text{last two inputs}} + \underbrace{\delta_{n} - \delta_{n + 1}}_{\text{current two inputs}}) \\
        e_{n+3}^{+\frac{1}{2}\pi} & = P_{n + 2}^{-\frac{3}{2}\pi} - P_{n + 3}^{+\frac{1}{2}\pi} =                                                                                                                  \\
