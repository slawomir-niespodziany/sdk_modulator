...
Utilizing the observation, that the scale factor shall not vary by more than a few per cent during the device's lifetime, it can be concluded that if the DAC was $(n+1)$--bit, the performance would not be affected. Its voltage range would have to be set for the worst--case scenario plus some margin, to always be sufficient, but the full scale wouldn't be used at any time. Even if the scale factor dropped to 50\% of the maximum range, which is beyond any pessimistic scenario, the overall performance would remain no worse than when using a $n$--bit DAC. From this an all--digital architecture can be defined.
...
\noindent
The $V\pi^{max}$ is the highest expected half--wave voltage. When determining the final reference it is advantageous to round it up to a voltage multiple of one of typically available chips (Equation~\ref{eq:reference_voltage_ceil}), e.g. $N \cdot 5V$ or $N \cdot 4.096V$. It makes implementing the elctronic circuit more straightfroward. In practice, a compromise is to utilize the resulting excess for the margin $M$, so that after adding it, the $V_{ref} \approx V_{ref}^{min}$.
...
\section{Algorithmic resolution enhancement} % Precision estimation and impact
TODO \\
