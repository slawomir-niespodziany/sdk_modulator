...
{Niespodziany \MakeLowercase{\textit{et al.}}: Algorithmic optimization for maximizing scale factor control precision with all--digital modulator architecture for an iFOG}
\definecolor{abstractbg}{rgb}{0.89804,0.94510,0.83137}
\setlength{\fboxrule}{0pt}
\setlength{\fboxsep}{0pt}

\begin{document}
\title{Algorithmic optimization for maximizing scale factor control precision with all--digital modulator architecture for an iFOG}
\author{Sławomir Niespodziany, Przemysław Rokita, \IEEEmembership{Member, IEEE}
    \thanks{The authors are with the Digital Multimedia Division, Institute of Computer Science, Warsaw University of Technology (e-mails: slawomir.niespodziany@pw.edu.pl, przemyslaw.rokita@pw.edu.pl).}
}

\IEEEtitleabstractindextext{%
    \fcolorbox{abstractbg}{abstractbg}{%
        \begin{minipage}{\textwidth}%
            \begin{wrapfigure}[15]{r}{4.2in}%
                \includegraphics[width=4in]{img/bias_abstract.png}\hspace{0.2in}%
            \end{wrapfigure}%
            \begin{abstract}
...
iFOG (Interferometric Fibre Optic Gyroscope \cite{culshaw2006optical, bergh1984overview}) concept is based on the Sagnac effect, discovered in 1913 \cite{sagnac1913ether, post1967sagnac}. Over the years, its design has been implemented and iterated multiple times, making it well refined. However, the limits which hold the development back are constantly pushed forward as technology evolves \cite{ma2020configuration}. This allows for better precision, miniaturized size, lower power consumption, etc. At the same time, the ever progressing industry finds new and more strict applications for the gyroscopes, setting the requirements bar even higher. Such a context drives the already existing designs to be tested and optimized in every possible aspect \cite{liadvances, smailov2026fiber, tashtay2025fiber}. This applies equally to all the optical and electro--optic components, to the electronics, and to the driving software. A crucial part of iFOG control system is the modulator. In a closed--loop design \cite{pavlath1996closed} it is a vital element of the feedback loop, directly affecting overall performance. It drives the optical circuit into a predefined state, in which the circuit's response can be demodulated and error signals determined. 
...
In \textit{closed--loop} operation \cite{lefevre1993fiber, khoshki2014investigation, ccelikel2009design, yan2021research} the control system biases the optical circuit sequentially in four predefined points $\Delta\phi \in \{+\frac{3}{2}\pi, -\frac{1}{2}\pi, -\frac{3}{2}\pi, +\frac{1}{2}\pi\}$, utilizing \textit{four--step} modulation \cite{zhang2024six, zhang2018six}, depicted in Figure~\ref{fig:bias}. This allows for linearizing the response, keeping the circuit at maximum sensitivity and detecting rotation direction.
...
The implementation also uses fixed--point arithmetic to perform calculations with fractional precision. The exemplary format used is Q6.2, where the values $\pm\frac{1}{4}$ and $\pm\frac{3}{4}$ are simply expressed as integers $\pm1$ and $\pm3$. For the final result the fractional bits are dropped using right--shift operation. It has to be noted, that fixed--point arithmetic is an optimal choice for this implementation, as the algorithm does not deal with values of high dynamic range and numerical determinism is of paramount importance for it. Moreover, a floating--point format would utilize more resources - both in the context of performing operations (addition, multiplication, etc.) and also for value storage (it requires additional bits for storing both mantissa and exponent). 
...
int8_t dac[4n+1] = (scaleFactor[4n+0] *  0x1) >> 2 = 3;
    ...
    \end{verbatim}}
    \caption{Pseudocode for controlling the scale factor for the current two bias points while accounting for the values driven previously. It is implemented using Q6.2 fixed--point format, for which rounding towards $-\inf$ is a native behavior.}
    \label{fig:snippet_implementation}
\end{figure}

In this example the effective drive corresponds to the scale factor, intended for the latter two bias points. The previous drive pair also had to be accounted for, to get this outcome

\begin{equation}
    ((3) - (-4)) - ((-12) - (9)) = 28 \quad [\Delta\phi_{v}] \text{.}
\end{equation}

Another possible approach would be to simply execute the control loop once every four samples. This results in very straightforward implementation, which apriori determines all four drive levels and then applies them. However, main drawback of this solution is that only two of four bias points from (\ref{eq:bias_order}) are usable for both scale factor and rotation calculation. This results from the dependence of the rotation measurement accuracy on the scale factor being controlled between each consecutive bias points pair. A detailed analysis of this matter remains a subject for further work. 

\section{Conclusions and further work}
An all--digital hardware architecture was presented and advocated for, as it offers the greatest potential for performance improvements. It is achieved through reduced analog complexity and software--defined flexibility. The key finding of the conducted analysis is that the scale factor feedback signal depends on four consecutive drive samples rather than two — a non--obvious result with direct consequences for algorithm correctness. Three implementation pitfalls were identified, and an optimal fixed--point algorithm was derived that recovers one bit of effective resolution through correct sample accounting and rounding. This work requires continuation: the mathematical analysis must also be applied to rotation calculation — in particular, the effect of generating a ramp signal with Serrodyne modulation — intentionally omitted at this time. Furthermore, a complete analysis including the demodulation process and control loop closure needs to be provided to ensure the whole design is well optimized — because the system is as performant as its least performing element.

\begin{thebibliography}{00}
    \bibitem{culshaw2006optical} Culshaw, Brian. "The optical fibre Sagnac interferometer: an overview of its principles and applications." Measurement Science and Technology 17.1 (2006): R1-R16.
    \bibitem{bergh1984overview} Bergh, R., H. Lefevre, and H. Shaw. "An overview of fiber-optic gyroscopes." Journal of Lightwave Technology 2.2 (1984): 91-107.
    ...
    \bibitem{ma2020configuration} Ma, Kun, et al. "Configuration optimization in miniature interferometric fiber-optic gyroscopes for space application." IEEE Sensors Journal 20.13 (2020): 7107-7117.
    \bibitem{liadvances} Li, Ruifeng, et al. "Advances in High-Precision Interferometric Fiber Optic Gyroscope." Advanced Devices \& Instrumentation.
    \bibitem{smailov2026fiber} Smailov, Nurzhigit, et al. "Fiber-Optic Gyroscopes in Modern Navigation Systems: A Comprehensive Review." Network 6.2 (2026): 28.
    \bibitem{tashtay2025fiber} Tashtay, Yerlan, et al. "Fiber-Optic Gyroscopes: Architectures, Signal Processing, Error Compensation, and Emerging Trends." Journal of Sensor and Actuator Networks 15.1 (2025): 3.
    ...
    \bibitem{khoshki2014investigation} Khoshki, Rohollah Mazrae, and Submarian Ganesan. "Investigation on Closed-loop Fiber Optic Gyroscope Structure and Operation." International Journal of Hybrid Information Technology 7.5 (2014): 23-32.
    \bibitem{ccelikel2009design} Çelikel, Oǧuz, and Sait Eren San. "Design details and characterization of all digital closed-loop interferometric fiber optic gyroscope with superluminescent light emitting diode." Optical review 16.1 (2009): 35-43.
    \bibitem{yan2021research} Yan, Jingtao, et al. "Research on the feedback control characteristics and parameter optimization of closed-loop fiber optic gyroscope." Optik 229 (2021): 166298.
    \bibitem{zhang2024six} Zhang, Tong, et al. "Six-state modulation method for improving the accuracy of fiber optic gyroscopes by reducing electrical crosstalk." Applied Optics 63.21 (2024): 5653-5660.
    \bibitem{zhang2018six} Zhang, Chunxi, et al. "Six-state phase modulation for reduced crosstalk in a fiber optic gyroscope." Optics express 26.8 (2018): 10535-10549.
    \bibitem{li2026analysis} Li, Tiezhi, et al. "Analysis and Compensation for the Impact of IOC Half-Wave Voltage Thermal Dependence Errors on Scale Factor of IFOGs." Journal of Lightwave Technology 44.7 (2026): 2714-2721.
...
\begin{IEEEbiographynophoto}{Sławomir Niespodziany}
    Academic researcher and engineer specializing in electronics and software development, with a distinct expertise in embedded systems. Affiliated with the Warsaw University of Technology, his work heavily intersects modern C++ practices with hardware design, focusing on creating modular, testable, and reusable architectures tailored specifically for resource-constrained electronic environments. Operating within the Digital Multimedia Division, his current research and engineering efforts focus primarily on FPGA-based designs, algorithm optimization, and computer vision research.
\end{IEEEbiographynophoto}

\begin{IEEEbiographynophoto}{Przemysław Rokita}
    Internationally recognized expert in computer graphics, image processing, and artificial intelligence. He currently holds the position of Full Professor at the Warsaw University of Technology, where he also leads the Digital Multimedia Division at the Faculty of Electronics and Information Technology. Has an extensive international academic background, having previously held research and teaching positions at leading institutions including the Max Planck Institute for Informatics (Germany), Hiroshima Institute of Technology (Japan), and Imperial College London (UK). He is an active member of several international program committees, including IEEE Information Visualization, Computer Graphics, and WSCG. He serves as a reviewer for top-tier journals such as IEEE Computer Graphics \& Applications, The Visual Computer, and IEEE Transactions on Circuits and Systems for Video Technology. Since 2020, member of the Committee on Computer Science of the Polish Academy of Sciences. Founder and first chairman of the Digital Multimedia Section of the Committee on Computer Science of the Polish Academy of Sciences.
\end{IEEEbiographynophoto}
\vfill
\end{document}
